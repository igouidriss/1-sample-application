...
import static com.mycompany.myapp.domain.Table8Asserts.*;
...
/**
 * Integration tests for the {@link Table8Resource} REST controller.
 */
@IntegrationTest
@AutoConfigureMockMvc
@WithMockUser
class Table8ResourceIT {
    ...
    private static final String ENTITY_API_URL = "/api/table-8-s";
    ...
    @Autowired
    private MockMvc restTable8MockMvc;
     ...
     */
    public static Table8 createEntity(EntityManager em) {
     ...
     * if they test an entity which requires the current entity.
     */
    public static Table8 createUpdatedEntity(EntityManager em) {
        Table8 table8 = new Table8()
    ...
    public void initTest() {
        table8 = createEntity(em);
    }

    @Test
    @Transactional
    void createTable8() throws Exception {
        long databaseSizeBeforeCreate = getRepositoryCount();
        // Create the Table8
        var returnedTable8 = om.readValue(
            restTable8MockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(table8)))
        ...
        // Validate the Table8 in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        assertTable8UpdatableFieldsEquals(returnedTable8, getPersistedTable8(returnedTable8));
    }

    @Test
    @Transactional
    void createTable8WithExistingId() throws Exception {
        // Create the Table8 with an existing ID
        table8.setId(1L);
    ...
    @Test
    @Transactional
    void getAllTable8s() throws Exception {
        // Initialize the database
        table8Repository.saveAndFlush(table8);

        // Get all the table8List
        restTable8MockMvc
            .perform(get(ENTITY_API_URL + "?sort=id,desc"))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(table8.getId().intValue())))
    ...
    @Test
    @Transactional
    void getTable8() throws Exception {
        // Initialize the database
        table8Repository.saveAndFlush(table8);

        // Get the table8
        restTable8MockMvc
            .perform(get(ENTITY_API_URL_ID, table8.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.id").value(table8.getId().intValue()))
    ...
    @Test
    @Transactional
    void getNonExistingTable8() throws Exception {
        // Get the table8
        restTable8MockMvc.perform(get(ENTITY_API_URL_ID, Long.MAX_VALUE)).andExpect(status().isNotFound());
    }

    @Test
    @Transactional
    void putExistingTable8() throws Exception {
        // Initialize the database
        table8Repository.saveAndFlush(table8);
        ...
        // Update the table8
        Table8 updatedTable8 = table8Repository.findById(table8.getId()).orElseThrow();
        // Disconnect from session so that the updates on updatedTable8 are not directly saved in db
        em.detach(updatedTable8);
        updatedTable8
            .numLigne1(UPDATED_NUM_LIGNE_1)
            .resort2(UPDATED_RESORT_2)
            .resvNameId3(UPDATED_RESV_NAME_ID_3)
        ...
        restTable8MockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedTable8.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedTable8))
            )
            .andExpect(status().isOk());

        // Validate the Table8 in the database
        assertSameRepositoryCount(databaseSizeBeforeUpdate);
        assertPersistedTable8ToMatchAllProperties(updatedTable8);
    }

    @Test
    @Transactional
    void putNonExistingTable8() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table8.setId(longCount.incrementAndGet());

        // If the entity doesn't have an ID, it will throw BadRequestAlertException
        restTable8MockMvc
            .perform(put(ENTITY_API_URL_ID, table8.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(table8)))
            .andExpect(status().isBadRequest());
    ...
    @Test
    @Transactional
    void putWithIdMismatchTable8() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table8.setId(longCount.incrementAndGet());
        ...
        restTable8MockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(table8))
            )
            .andExpect(status().isBadRequest());
    ...
    @Test
    @Transactional
    void putWithMissingIdPathParamTable8() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table8.setId(longCount.incrementAndGet());

        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restTable8MockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(table8)))
    ...
    @Test
    @Transactional
    void partialUpdateTable8WithPatch() throws Exception {
        // Initialize the database
        table8Repository.saveAndFlush(table8);

        long databaseSizeBeforeUpdate = getRepositoryCount();

        // Update the table8 using partial update
        Table8 partialUpdatedTable8 = new Table8();
        partialUpdatedTable8.setId(table8.getId());

        partialUpdatedTable8
            .numLigne1(UPDATED_NUM_LIGNE_1)
            .resvNameId3(UPDATED_RESV_NAME_ID_3)
            .clientOId4(UPDATED_CLIENT_O_ID_4)
            .nameId8(UPDATED_NAME_ID_8)
        ...
        restTable8MockMvc
            .perform(
                patch(ENTITY_API_URL_ID, partialUpdatedTable8.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(partialUpdatedTable8))
            )
            .andExpect(status().isOk());
        ...
        assertSameRepositoryCount(databaseSizeBeforeUpdate);
        assertTable8UpdatableFieldsEquals(createUpdateProxyForBean(partialUpdatedTable8, table8), getPersistedTable8(table8));
    }

    @Test
    @Transactional
    void fullUpdateTable8WithPatch() throws Exception {
        // Initialize the database
        table8Repository.saveAndFlush(table8);
        ...
        assertSameRepositoryCount(databaseSizeBeforeUpdate);
        assertTable8UpdatableFieldsEquals(partialUpdatedTable8, getPersistedTable8(partialUpdatedTable8));
    }

    @Test
    @Transactional
    void patchNonExistingTable8() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table8.setId(longCount.incrementAndGet());
        ...
        restTable8MockMvc
            .perform(
                patch(ENTITY_API_URL_ID, table8.getId()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(table8))
            )
            .andExpect(status().isBadRequest());
    ...
    @Test
    @Transactional
    void patchWithIdMismatchTable8() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table8.setId(longCount.incrementAndGet());
    ...
    @Test
    @Transactional
    void patchWithMissingIdPathParamTable8() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table8.setId(longCount.incrementAndGet());

        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restTable8MockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(table8)))
            .andExpect(status().isMethodNotAllowed());
    ...
    @Test
    @Transactional
    void deleteTable8() throws Exception {
        // Initialize the database
        table8Repository.saveAndFlush(table8);
        ...
        // Delete the table8
        restTable8MockMvc
            .perform(delete(ENTITY_API_URL_ID, table8.getId()).accept(MediaType.APPLICATION_JSON))
    ...
    protected Table8 getPersistedTable8(Table8 table8) {
        return table8Repository.findById(table8.getId()).orElseThrow();
    }

    protected void assertPersistedTable8ToMatchAllProperties(Table8 expectedTable8) {
        assertTable8AllPropertiesEquals(expectedTable8, getPersistedTable8(expectedTable8));
    }

    protected void assertPersistedTable8ToMatchUpdatableProperties(Table8 expectedTable8) {
        assertTable8AllUpdatablePropertiesEquals(expectedTable8, getPersistedTable8(expectedTable8));
    }
}
