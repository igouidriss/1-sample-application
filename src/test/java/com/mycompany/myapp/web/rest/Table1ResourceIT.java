...
import static com.mycompany.myapp.domain.Table1Asserts.*;
...
class Table1ResourceIT {
    ...
    private static final String ENTITY_API_URL = "/api/table-1-s";
    ...
    void createTable1() throws Exception {
        ...
        // Create the Table1
        var returnedTable1 = om.readValue(
            restTable1MockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(table1)))
        ...
        assertTable1UpdatableFieldsEquals(returnedTable1, getPersistedTable1(returnedTable1));
    }

    @Test
    @Transactional
    void createTable1WithExistingId() throws Exception {
    ...
    @Test
    @Transactional
    void getAllTable1s() throws Exception {
    ...
    @Test
    @Transactional
    void getTable1() throws Exception {
        // Initialize the database
        table1Repository.saveAndFlush(table1);

        // Get the table1
        restTable1MockMvc
            .perform(get(ENTITY_API_URL_ID, table1.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
    ...
    @Test
    @Transactional
    void getNonExistingTable1() throws Exception {
        // Get the table1
        restTable1MockMvc.perform(get(ENTITY_API_URL_ID, Long.MAX_VALUE)).andExpect(status().isNotFound());
    }

    @Test
    @Transactional
    void putExistingTable1() throws Exception {
        // Initialize the database
        table1Repository.saveAndFlush(table1);
        ...
        // Update the table1
        Table1 updatedTable1 = table1Repository.findById(table1.getId()).orElseThrow();
        ...
        restTable1MockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedTable1.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedTable1))
            )
            .andExpect(status().isOk());

        // Validate the Table1 in the database
        assertSameRepositoryCount(databaseSizeBeforeUpdate);
        assertPersistedTable1ToMatchAllProperties(updatedTable1);
    }

    @Test
    @Transactional
    void putNonExistingTable1() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table1.setId(longCount.incrementAndGet());

        // If the entity doesn't have an ID, it will throw BadRequestAlertException
        restTable1MockMvc
            .perform(put(ENTITY_API_URL_ID, table1.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(table1)))
            .andExpect(status().isBadRequest());
    ...
    @Test
    @Transactional
    void putWithIdMismatchTable1() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table1.setId(longCount.incrementAndGet());
        ...
        restTable1MockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(table1))
            )
            .andExpect(status().isBadRequest());
    ...
    @Test
    @Transactional
    void putWithMissingIdPathParamTable1() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table1.setId(longCount.incrementAndGet());

        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restTable1MockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(table1)))
    ...
    @Test
    @Transactional
    void partialUpdateTable1WithPatch() throws Exception {
        // Initialize the database
        table1Repository.saveAndFlush(table1);

        long databaseSizeBeforeUpdate = getRepositoryCount();

        // Update the table1 using partial update
        Table1 partialUpdatedTable1 = new Table1();
        partialUpdatedTable1.setId(table1.getId());

        partialUpdatedTable1
            .numLigne1(UPDATED_NUM_LIGNE_1)
            .clientOId4(UPDATED_CLIENT_O_ID_4)
            .typeChb5(UPDATED_TYPE_CHB_5)
            .confirmationNo6(UPDATED_CONFIRMATION_NO_6)
        ...
        restTable1MockMvc
            .perform(
                patch(ENTITY_API_URL_ID, partialUpdatedTable1.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(partialUpdatedTable1))
            )
            .andExpect(status().isOk());
        ...
        assertSameRepositoryCount(databaseSizeBeforeUpdate);
        assertTable1UpdatableFieldsEquals(createUpdateProxyForBean(partialUpdatedTable1, table1), getPersistedTable1(table1));
    }

    @Test
    @Transactional
    void fullUpdateTable1WithPatch() throws Exception {
        // Initialize the database
        table1Repository.saveAndFlush(table1);
        ...
        assertSameRepositoryCount(databaseSizeBeforeUpdate);
        assertTable1UpdatableFieldsEquals(partialUpdatedTable1, getPersistedTable1(partialUpdatedTable1));
    }

    @Test
    @Transactional
    void patchNonExistingTable1() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table1.setId(longCount.incrementAndGet());
        ...
        restTable1MockMvc
            .perform(
                patch(ENTITY_API_URL_ID, table1.getId()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(table1))
            )
            .andExpect(status().isBadRequest());
    ...
    @Test
    @Transactional
    void patchWithIdMismatchTable1() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table1.setId(longCount.incrementAndGet());
    ...
    @Test
    @Transactional
    void patchWithMissingIdPathParamTable1() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table1.setId(longCount.incrementAndGet());

        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restTable1MockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(table1)))
            .andExpect(status().isMethodNotAllowed());
    ...
    @Test
    @Transactional
    void deleteTable1() throws Exception {
        // Initialize the database
        table1Repository.saveAndFlush(table1);
        ...
        // Delete the table1
        restTable1MockMvc
            .perform(delete(ENTITY_API_URL_ID, table1.getId()).accept(MediaType.APPLICATION_JSON))
    ...
    protected Table1 getPersistedTable1(Table1 table1) {
        return table1Repository.findById(table1.getId()).orElseThrow();
    }

    protected void assertPersistedTable1ToMatchAllProperties(Table1 expectedTable1) {
        assertTable1AllPropertiesEquals(expectedTable1, getPersistedTable1(expectedTable1));
    }

    protected void assertPersistedTable1ToMatchUpdatableProperties(Table1 expectedTable1) {
        assertTable1AllUpdatablePropertiesEquals(expectedTable1, getPersistedTable1(expectedTable1));
    }
}
