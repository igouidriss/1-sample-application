...
import static com.mycompany.myapp.domain.Table6Asserts.*;
...
/**
 * Integration tests for the {@link Table6Resource} REST controller.
 */
@IntegrationTest
@AutoConfigureMockMvc
@WithMockUser
class Table6ResourceIT {
    ...
    private static final String ENTITY_API_URL = "/api/table-6-s";
    ...
    @Autowired
    private MockMvc restTable6MockMvc;
     ...
     */
    public static Table6 createEntity(EntityManager em) {
     ...
     * if they test an entity which requires the current entity.
     */
    public static Table6 createUpdatedEntity(EntityManager em) {
        Table6 table6 = new Table6()
    ...
    public void initTest() {
        table6 = createEntity(em);
    }

    @Test
    @Transactional
    void createTable6() throws Exception {
        long databaseSizeBeforeCreate = getRepositoryCount();
        // Create the Table6
        var returnedTable6 = om.readValue(
            restTable6MockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(table6)))
        ...
        // Validate the Table6 in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        assertTable6UpdatableFieldsEquals(returnedTable6, getPersistedTable6(returnedTable6));
    }

    @Test
    @Transactional
    void createTable6WithExistingId() throws Exception {
        // Create the Table6 with an existing ID
        table6.setId(1L);
    ...
    @Test
    @Transactional
    void getAllTable6s() throws Exception {
        // Initialize the database
        table6Repository.saveAndFlush(table6);

        // Get all the table6List
        restTable6MockMvc
            .perform(get(ENTITY_API_URL + "?sort=id,desc"))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(table6.getId().intValue())))
    ...
    @Test
    @Transactional
    void getTable6() throws Exception {
        // Initialize the database
        table6Repository.saveAndFlush(table6);

        // Get the table6
        restTable6MockMvc
            .perform(get(ENTITY_API_URL_ID, table6.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.id").value(table6.getId().intValue()))
    ...
    @Test
    @Transactional
    void getNonExistingTable6() throws Exception {
        // Get the table6
        restTable6MockMvc.perform(get(ENTITY_API_URL_ID, Long.MAX_VALUE)).andExpect(status().isNotFound());
    }

    @Test
    @Transactional
    void putExistingTable6() throws Exception {
        // Initialize the database
        table6Repository.saveAndFlush(table6);
        ...
        // Update the table6
        Table6 updatedTable6 = table6Repository.findById(table6.getId()).orElseThrow();
        // Disconnect from session so that the updates on updatedTable6 are not directly saved in db
        em.detach(updatedTable6);
        updatedTable6
            .numLigne1(UPDATED_NUM_LIGNE_1)
            .resort2(UPDATED_RESORT_2)
            .resvNameId3(UPDATED_RESV_NAME_ID_3)
        ...
        restTable6MockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedTable6.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedTable6))
            )
            .andExpect(status().isOk());

        // Validate the Table6 in the database
        assertSameRepositoryCount(databaseSizeBeforeUpdate);
        assertPersistedTable6ToMatchAllProperties(updatedTable6);
    }

    @Test
    @Transactional
    void putNonExistingTable6() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table6.setId(longCount.incrementAndGet());

        // If the entity doesn't have an ID, it will throw BadRequestAlertException
        restTable6MockMvc
            .perform(put(ENTITY_API_URL_ID, table6.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(table6)))
            .andExpect(status().isBadRequest());
    ...
    @Test
    @Transactional
    void putWithIdMismatchTable6() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table6.setId(longCount.incrementAndGet());
        ...
        restTable6MockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(table6))
            )
            .andExpect(status().isBadRequest());
    ...
    @Test
    @Transactional
    void putWithMissingIdPathParamTable6() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table6.setId(longCount.incrementAndGet());

        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restTable6MockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(table6)))
    ...
    @Test
    @Transactional
    void partialUpdateTable6WithPatch() throws Exception {
        // Initialize the database
        table6Repository.saveAndFlush(table6);

        long databaseSizeBeforeUpdate = getRepositoryCount();

        // Update the table6 using partial update
        Table6 partialUpdatedTable6 = new Table6();
        partialUpdatedTable6.setId(table6.getId());

        partialUpdatedTable6
            .numLigne1(UPDATED_NUM_LIGNE_1)
            .resvNameId3(UPDATED_RESV_NAME_ID_3)
            .clientOId4(UPDATED_CLIENT_O_ID_4)
            .typeChb5(UPDATED_TYPE_CHB_5)
        ...
        restTable6MockMvc
            .perform(
                patch(ENTITY_API_URL_ID, partialUpdatedTable6.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(partialUpdatedTable6))
            )
            .andExpect(status().isOk());
        ...
        assertSameRepositoryCount(databaseSizeBeforeUpdate);
        assertTable6UpdatableFieldsEquals(createUpdateProxyForBean(partialUpdatedTable6, table6), getPersistedTable6(table6));
    }

    @Test
    @Transactional
    void fullUpdateTable6WithPatch() throws Exception {
        // Initialize the database
        table6Repository.saveAndFlush(table6);
        ...
        assertSameRepositoryCount(databaseSizeBeforeUpdate);
        assertTable6UpdatableFieldsEquals(partialUpdatedTable6, getPersistedTable6(partialUpdatedTable6));
    }

    @Test
    @Transactional
    void patchNonExistingTable6() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table6.setId(longCount.incrementAndGet());
        ...
        restTable6MockMvc
            .perform(
                patch(ENTITY_API_URL_ID, table6.getId()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(table6))
            )
            .andExpect(status().isBadRequest());
    ...
    @Test
    @Transactional
    void patchWithIdMismatchTable6() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table6.setId(longCount.incrementAndGet());
    ...
    @Test
    @Transactional
    void patchWithMissingIdPathParamTable6() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table6.setId(longCount.incrementAndGet());

        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restTable6MockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(table6)))
            .andExpect(status().isMethodNotAllowed());
    ...
    @Test
    @Transactional
    void deleteTable6() throws Exception {
        // Initialize the database
        table6Repository.saveAndFlush(table6);
        ...
        // Delete the table6
        restTable6MockMvc
            .perform(delete(ENTITY_API_URL_ID, table6.getId()).accept(MediaType.APPLICATION_JSON))
    ...
    protected Table6 getPersistedTable6(Table6 table6) {
        return table6Repository.findById(table6.getId()).orElseThrow();
    }

    protected void assertPersistedTable6ToMatchAllProperties(Table6 expectedTable6) {
        assertTable6AllPropertiesEquals(expectedTable6, getPersistedTable6(expectedTable6));
    }

    protected void assertPersistedTable6ToMatchUpdatableProperties(Table6 expectedTable6) {
        assertTable6AllUpdatablePropertiesEquals(expectedTable6, getPersistedTable6(expectedTable6));
    }
}
