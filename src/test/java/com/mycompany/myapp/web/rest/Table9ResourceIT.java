...
import static com.mycompany.myapp.domain.Table9Asserts.*;
...
/**
 * Integration tests for the {@link Table9Resource} REST controller.
 */
@IntegrationTest
@AutoConfigureMockMvc
@WithMockUser
class Table9ResourceIT {
    ...
    private static final String ENTITY_API_URL = "/api/table-9-s";
    ...
    @Autowired
    private MockMvc restTable9MockMvc;
     ...
     */
    public static Table9 createEntity(EntityManager em) {
     ...
     * if they test an entity which requires the current entity.
     */
    public static Table9 createUpdatedEntity(EntityManager em) {
        Table9 table9 = new Table9()
    ...
    public void initTest() {
        table9 = createEntity(em);
    }

    @Test
    @Transactional
    void createTable9() throws Exception {
        long databaseSizeBeforeCreate = getRepositoryCount();
        // Create the Table9
        var returnedTable9 = om.readValue(
            restTable9MockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(table9)))
        ...
        // Validate the Table9 in the database
        assertIncrementedRepositoryCount(databaseSizeBeforeCreate);
        assertTable9UpdatableFieldsEquals(returnedTable9, getPersistedTable9(returnedTable9));
    }

    @Test
    @Transactional
    void createTable9WithExistingId() throws Exception {
        // Create the Table9 with an existing ID
        table9.setId(1L);
    ...
    @Test
    @Transactional
    void getAllTable9s() throws Exception {
        // Initialize the database
        table9Repository.saveAndFlush(table9);

        // Get all the table9List
        restTable9MockMvc
            .perform(get(ENTITY_API_URL + "?sort=id,desc"))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(table9.getId().intValue())))
    ...
    @Test
    @Transactional
    void getTable9() throws Exception {
        // Initialize the database
        table9Repository.saveAndFlush(table9);

        // Get the table9
        restTable9MockMvc
            .perform(get(ENTITY_API_URL_ID, table9.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.id").value(table9.getId().intValue()))
    ...
    @Test
    @Transactional
    void getNonExistingTable9() throws Exception {
        // Get the table9
        restTable9MockMvc.perform(get(ENTITY_API_URL_ID, Long.MAX_VALUE)).andExpect(status().isNotFound());
    }

    @Test
    @Transactional
    void putExistingTable9() throws Exception {
        // Initialize the database
        table9Repository.saveAndFlush(table9);
        ...
        // Update the table9
        Table9 updatedTable9 = table9Repository.findById(table9.getId()).orElseThrow();
        // Disconnect from session so that the updates on updatedTable9 are not directly saved in db
        em.detach(updatedTable9);
        updatedTable9
            .numLigne1(UPDATED_NUM_LIGNE_1)
            .resort2(UPDATED_RESORT_2)
            .resvNameId3(UPDATED_RESV_NAME_ID_3)
        ...
        restTable9MockMvc
            .perform(
                put(ENTITY_API_URL_ID, updatedTable9.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(updatedTable9))
            )
            .andExpect(status().isOk());

        // Validate the Table9 in the database
        assertSameRepositoryCount(databaseSizeBeforeUpdate);
        assertPersistedTable9ToMatchAllProperties(updatedTable9);
    }

    @Test
    @Transactional
    void putNonExistingTable9() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table9.setId(longCount.incrementAndGet());

        // If the entity doesn't have an ID, it will throw BadRequestAlertException
        restTable9MockMvc
            .perform(put(ENTITY_API_URL_ID, table9.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(table9)))
            .andExpect(status().isBadRequest());
    ...
    @Test
    @Transactional
    void putWithIdMismatchTable9() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table9.setId(longCount.incrementAndGet());
        ...
        restTable9MockMvc
            .perform(
                put(ENTITY_API_URL_ID, longCount.incrementAndGet())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(om.writeValueAsBytes(table9))
            )
            .andExpect(status().isBadRequest());
    ...
    @Test
    @Transactional
    void putWithMissingIdPathParamTable9() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table9.setId(longCount.incrementAndGet());

        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restTable9MockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(table9)))
    ...
    @Test
    @Transactional
    void partialUpdateTable9WithPatch() throws Exception {
        // Initialize the database
        table9Repository.saveAndFlush(table9);

        long databaseSizeBeforeUpdate = getRepositoryCount();

        // Update the table9 using partial update
        Table9 partialUpdatedTable9 = new Table9();
        partialUpdatedTable9.setId(table9.getId());

        partialUpdatedTable9
            .numLigne1(UPDATED_NUM_LIGNE_1)
            .resort2(UPDATED_RESORT_2)
            .clientOId4(UPDATED_CLIENT_O_ID_4)
            .nameId8(UPDATED_NAME_ID_8)
        ...
        restTable9MockMvc
            .perform(
                patch(ENTITY_API_URL_ID, partialUpdatedTable9.getId())
                    .contentType("application/merge-patch+json")
                    .content(om.writeValueAsBytes(partialUpdatedTable9))
            )
            .andExpect(status().isOk());
        ...
        assertSameRepositoryCount(databaseSizeBeforeUpdate);
        assertTable9UpdatableFieldsEquals(createUpdateProxyForBean(partialUpdatedTable9, table9), getPersistedTable9(table9));
    }

    @Test
    @Transactional
    void fullUpdateTable9WithPatch() throws Exception {
        // Initialize the database
        table9Repository.saveAndFlush(table9);
        ...
        assertSameRepositoryCount(databaseSizeBeforeUpdate);
        assertTable9UpdatableFieldsEquals(partialUpdatedTable9, getPersistedTable9(partialUpdatedTable9));
    }

    @Test
    @Transactional
    void patchNonExistingTable9() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table9.setId(longCount.incrementAndGet());
        ...
        restTable9MockMvc
            .perform(
                patch(ENTITY_API_URL_ID, table9.getId()).contentType("application/merge-patch+json").content(om.writeValueAsBytes(table9))
            )
            .andExpect(status().isBadRequest());
    ...
    @Test
    @Transactional
    void patchWithIdMismatchTable9() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table9.setId(longCount.incrementAndGet());
    ...
    @Test
    @Transactional
    void patchWithMissingIdPathParamTable9() throws Exception {
        long databaseSizeBeforeUpdate = getRepositoryCount();
        table9.setId(longCount.incrementAndGet());

        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restTable9MockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(table9)))
            .andExpect(status().isMethodNotAllowed());
    ...
    @Test
    @Transactional
    void deleteTable9() throws Exception {
        // Initialize the database
        table9Repository.saveAndFlush(table9);
        ...
        // Delete the table9
        restTable9MockMvc
            .perform(delete(ENTITY_API_URL_ID, table9.getId()).accept(MediaType.APPLICATION_JSON))
    ...
    protected Table9 getPersistedTable9(Table9 table9) {
        return table9Repository.findById(table9.getId()).orElseThrow();
    }

    protected void assertPersistedTable9ToMatchAllProperties(Table9 expectedTable9) {
        assertTable9AllPropertiesEquals(expectedTable9, getPersistedTable9(expectedTable9));
    }

    protected void assertPersistedTable9ToMatchUpdatableProperties(Table9 expectedTable9) {
        assertTable9AllUpdatablePropertiesEquals(expectedTable9, getPersistedTable9(expectedTable9));
    }
}
